...
void main() {
  ...
  group('OutsideEventOrganizerService', () {
    test('cleans HTML, summarizes, infers tags, and marks missing fields', () {
      ...
      expect(
        organized.displaySummary,
        'A guided outdoor walk & local history.',
      );
      expect(organized.tags, contains('outdoors'));
      expect(
        organized.missingFields,
  ...
  group('OutsideEventDiscoveryService', () {
    test(
      'combines source results, dedupes, sorts, and keeps warnings',
      () async {
        ...
        final service = OutsideEventDiscoveryService(
          adapters: [
            _FixedAdapter(id: 'a', displayName: 'A', events: [later, event]),
            _FixedAdapter(
              id: 'b',
              displayName: 'B',
              events: [duplicate],
    ...
    test(
      'tracks which sources were attempted and how many events each found',
      () async {
        final start = DateTime(2026, 7, 1);
        final service = OutsideEventDiscoveryService(
          adapters: [
...
''',
      ...
      expect(
        result.suggestions.single.startDateTime,
        DateTime(2026, 6, 28, 19, 30),
      );
      expect(result.suggestions.single.tags, contains('music'));
      expect(
        result.suggestions.single.missingFields,
        containsAll(['address', 'price', 'venue']),
      );
    });
...
''', 200);
      ...
      final result = await adapter.fetch(
        OutsideEventQuery(start: DateTime(2026, 7), end: DateTime(2026, 7, 7)),
      );
...
''', 200);
  ...
  group('User-managed event source adapters', () {
    test(
      'loads user RSS URLs through the Netlify proxy url parameter',
      () async {
...
''', 200);
        ...
        expect(
          result.suggestions.single.sourceType,
          OutsideEventSourceType.rssAtom,
        );
    ...
    test('loads webpage URLs through the webpage Netlify function', () async {
      final adapter = WebPageEventSourceAdapter(
        ...
        client: MockClient((request) async {
          expect(
            request.url.path,
            '/.netlify/functions/outside-events-webpage',
          );
          expect(
            request.url.queryParameters['url'],
...
''', 200);
      ...
      expect(
        result.suggestions.single.sourceType,
        OutsideEventSourceType.webPage,
      );
...
EventSuggestion _event({
  ...
  return EventSuggestion(
    ...
    dedupeKey: eventDedupeKey(title: title, start: start, venueName: 'Venue'),
  );
}
