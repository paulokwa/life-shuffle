...
class _OutsideEventsScreenState extends State<OutsideEventsScreen> {
                    ...
                    builder: (context, snapshot) {
                      ...
                      if (snapshot.hasError) {
                        return const _MessageCard(
                          ...
                          body:
                              'Try refresh. The rest of the planner is '
                              'unchanged.',
                        ...
                        children: [
                          ...
                          if (events.isEmpty)
                            const _MessageCard(
                              ...
                              body:
                                  'Try clearing filters or refresh later. '
  ...
  bool _isAdded(EventSuggestion event) {
    final state = AppStateScope.of(context);
    return state.manualPlanItems.any((item) => item.outsideEventId == event.id);
  }
...
class _SourceStatusCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final configured = result.sources
        .where((source) => source.configured)
        .length;
    return LsCard(
