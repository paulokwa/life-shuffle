...
class OutsideEventOrganizerService {
  ...
  static const bool aiConfigured =
      bool.fromEnvironment('OUTSIDE_EVENTS_AI_ORGANIZER');
  ...
  EventSuggestion organize(EventSuggestion event) {
    ...
    return event.copyWith(
      ...
      description:
          cleanDescription.isEmpty ? event.description : cleanDescription,
      summary: summary,
      tags: tags,
  ...
  static String _cleanTitle(String value) {
    ...
    return stripped
        .replaceAll(
            RegExp(r'\s*[-|]\s*(event|tickets?|halifax)$',
                caseSensitive: false),
            '')
        .replaceAll(RegExp(r'\s+'), ' ')
        .trim();
  ...
  static List<String> _inferTags(String text,
      {required List<String> existing}) {
    final lower = text.toLowerCase();
    final tags = <String>{
      ...existing.map((tag) => tag.trim()).where(
            (tag) => tag.isNotEmpty,
          )
    };
    ...
    addIf(
        RegExp(r'\bart|film|gallery|museum|culture|ceramic\b'), 'arts/culture');
