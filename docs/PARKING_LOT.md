...
- **Status update (2026-06-29)**: Slice 9 added the persisted `PlanHistoryEntry` foundation, Slice 10 uses it for existing Progress summaries, Slice 11 added archived-day details, Slice 12 added bounded previous-week and previous-month browsing with archive-only summaries, Slice 13 added ranked per-category stats (planned/done/partly/skipped/unchecked counts and weighted completion percentage) to the week and month views, and Slice 14 added current and longest completion streak cards (calculated from frozen archive data) to the week and month views. **Still parked**: year/N-day views, custom range pickers, calendar-grid history, and richer long-term charts or trends.
- **Why it is parked**: Day, week, and month list browsing with honest summaries now exists. Calendar-grid history, arbitrary ranges, and analytics should wait for a concrete need and real archive-growth experience.
