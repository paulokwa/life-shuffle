...
## 2026-06-21 - `memberUserIds.hasAny([uid])` to `uid in data.memberUserIds` did not actually fix the list-query permission-denied

- **Context**: The `in` rewrite above (and a follow-up that also dropped the `isOwner` OR from `allow list`) was deployed to production and re-tested directly against live Firestore using a disposable throwaway calendar doc and `signInWithCustomToken` + the Firestore REST API (bypassing the need for a real browser sign-in). Both still returned `403 PERMISSION_DENIED` on the exact `calendars.where('memberUserIds', arrayContains: uid)` list query — even for a completely unrelated "stranger" UID that should have gotten an empty result, not a denial.
- **Symptoms**: Identical to the entry above (`Firestore permission denied` in Settings sync diagnostics), persisting across two separate rules deploys that each looked like they should have fixed it.
- **Cause**: The real blocker was a third function, `isMember()`'s `hasMemberList(data)` call (`data.memberUserIds is list`). For Firestore to allow a `list`/query request, it must statically prove every possible matched document satisfies the rule using only the fields the query itself filters on. A type-check (`is list`) on a field is not something Firestore's query-safety analysis can resolve from an `array-contains` filter, so the whole rule became unprovable and Firestore denied the query outright — independent of whether the membership check used `hasAny()` or `in`, and independent of the `isOwner` OR.
- **Fix**: Removed the `hasMemberList(data)` call from inside `isMember()`. `memberUserIds` is already guaranteed to be a list by `allow create`'s `hasMemberList(request.resource.data)` check at document-creation time, so the runtime type-check inside `isMember()` was redundant defense that broke query provability. `allow list` keeps the `isOwner(resource.data) || isMember(resource.data)` form (verified this still works with the type-check gone).
- **Files affected**:
  - `firestore.rules`
- **Prevention / future note**: When a Firestore `list`/query rule keeps returning `permission-denied` despite using a documented-safe pattern (`field_value in resource.data.field`), check every function the rule calls for any `is list`/`is map`/type-check condition on the filtered field — those break static provability just as much as checking an unrelated field does. Verified directly against production using `firebase-admin.auth().createCustomToken()` + `signInWithCustomToken` (REST, with a `Referer` header matching an allowed origin) + the Firestore REST `runQuery` endpoint against a disposable throwaway document — this avoids needing a real browser/Google sign-in to test rules changes and is faster than the Firestore emulator when Java < 21 isn't available locally.
