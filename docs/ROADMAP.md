...
- [x] Slice 6 — focused plan-item occurrence editor: manual testing found that the Plan day sheet's edit action opened the full source activity/template editor, which is the wrong tool when the user is looking at one specific planned occurrence (e.g. "Cook together, Sunday 28 Jun, 8:00 PM, Couple time") and wants to tweak just that occurrence's time/category/dimensions. The day sheet's primary action is now "Edit this plan item," which opens a new focused `_PlanItemEditorSheet` (`plan_screen.dart`) showing only the actual scheduled time (free-text, validated against the app's existing `h:mm AM/PM` shape and normalized on save), category, and enabled planning dimensions (Difficulty/Energy/Social, each only when its Settings toggle is on) - never preferred time, max-per-week, allowed days, avoid-back-to-back-days, enabled/disabled, or any other source-template/global-rule field. A small secondary "Edit activity template" link (both on the day sheet card itself and inside the new editor) opens the existing full `_ActivityFormSheet` unchanged, with copy explaining it changes the reusable activity, including future plans. `PlannedActivity` gained mutable `categoryOverride`/`difficultyOverride`/`energyOverride`/`socialOverride` fields (alongside the already-mutable `timeSlot`) and override-aware `category`/`difficulty`/`energy`/`social` getters, so a per-occurrence edit never touches the source `Activity`. A new `AppState.editPlannedOccurrence(day, activity, {timeSlot, category, difficulty, energy, social})` applies the edit directly to the live `PlannedActivity` and records it in a new occurrence-keyed (`yyyy-MM-dd:activityId`) `_occurrenceOverrides` map, parallel to `_removedOccurrences`/`checkinMap`/`lockedMap`: a new `SavedState.occurrenceOverrides` field and `PersistenceService` blob save/load round-trip it, and a new `AppState._applyOccurrenceOverrides()` (parallel to `_applyOverlays`/`_applyRemovals`) reapplies it to a freshly built `_generatedDays` on load/sync. `PlannerService.timeRank` was generalized from a fixed 9-slot lookup table to a real `h:mm AM/PM` minutes-since-midnight parser (same relative order for every existing slot, unparseable input still sorts last) so a freely-entered occurrence time like "7:30 PM" sorts correctly everywhere `timeRank` is used. `TextWeekExportService.dimensionLabels` now takes raw `difficulty`/`energy`/`social` values instead of an `Activity`, and its three call sites (export text, and both print preview activity rows) now pass the override-aware `PlannedActivity` getters - so Copy text and print/PDF naturally reflect occurrence overrides since they already read from `generatedRange`/`exportDays`/`weekPlan`. `regenerate()`, `generateRange()`, and `setPlanStyle()` all clear `_occurrenceOverrides` (matching `_removedOccurrences`) since overrides belong to the generated plan being discarded.
- [x] Slice 6b — time-picker control and honest Check in affordances: manual testing of slice 6 found the occurrence editor's scheduled-time field was still a free-text box (clunky on desktop, worse on mobile), and separately that the Plan screen showed an active-looking "Check in" label on future days and on empty/rest days with nothing to check into. The occurrence editor's time field is now a tap-to-pick control (`GestureDetector` + `InputDecorator` showing the current time, keyed `plan-item-editor-time-field`) that opens Flutter's built-in `showTimePicker` (default dial entry mode, with the picker's own "switch to text input" toggle as the only remaining manual-typing path) rather than requiring typing as the primary interaction; the old `_planItemTimePattern` regex is kept, repurposed to parse the occurrence's existing `h:mm AM/PM` string into the picker's initial `TimeOfDay`, and a new formatter converts the picked `TimeOfDay` back to that same display shape on save. The Plan day card (`_DayBlock`), day strip (`_DayStrip`), and month grid (`_MonthGrid`/`_MonthDayCell`) now all gate check-in invitation on `plan.activities.isNotEmpty`: empty/rest days show no "Check in"/"Upcoming" label and are no longer tappable at all (no check-in sheet opens, since there is nothing to check in, edit, or remove), today/past days with activities keep the active "Check in" label exactly as before, and future days with activities show a muted, non-action "Upcoming" label while remaining tappable so "Edit this plan item"/"Edit activity template"/"Remove from this plan" stay reachable - only the day sheet's own Done/Partly/Skipped/Unchecked controls stay date-gated, unchanged from slice 3b. No change to generation behavior, date-moving, native PDF, or custom-horizon UI.
- [x] Slice 7 — manual "Add to plan" support: users can pin extra activities to a specific date/time without regenerating. A new `ManualPlanItem` model stores title, scheduled time, category, duration, enabled planning dimensions, optional source activity id, and a stable id. Manual items are serialized in `SavedState.manualPlanItems`, re-applied to `_generatedDays` after every build/regenerate/view switch, and excluded from `_collectLocked()` (they are pinned by `_applyManualItems()`, not by locked-generated-occurrence carry), so they survive `regenerate()`, `generateRange(...)`, `setPlanStyle(...)`, and reload/sync. They appear naturally in week/2-week/month views, day check-in sheets, text export, print preview, and the ICS feed because they are merged into `DayPlan.activities` as `PlannedActivity` instances backed by synthetic `Activity` objects (`manual_<itemId>`, `enabled: false`) that never collide with generated occurrences or get picked by the generator. The Plan screen exposes add affordances on every day card header, empty day strip cells, empty in-range month grid cells (a distinct plus icon so the cell body stays non-tappable), and inside empty day check-in sheets and check-in sheet headers. A new `_AddPlanItemSheet` lets users either create a one-off item or pick an existing library activity; picking an existing activity copies its values at creation time and does not link the manual item to future template edits. One-off items have an optional "Save to activity library for future use" checkbox that creates a normal `Activity` and records its id on the manual item. Removing a manual occurrence deletes the pinned item entirely; editing a manual occurrence updates the underlying `ManualPlanItem` so the change survives regeneration.
- [x] Slice 8 — "Must include in plans" activity priority: manual testing of slice 7 surfaced a separate planner gap — a high max-per-week activity with many allowed days (e.g. "Eat Together" at 6/week across 6 allowed days) wasn't guaranteed to appear that often, because the generator only ever treated activities as flexible suggestions chosen by least-used/difficulty-aware tie-breaking. Added a `mustIncludeInPlans` bool to `Activity` (defaults to `false`, preserved by `copy()`/`toMap()`/`fromMap()`, safe-defaulted for legacy saved data) and a matching "Must include in plans" toggle in the activity editor, placed next to max-per-week/allowed days since it's a generation rule, with helper text "The planner schedules this first, up to its max per week." The activity editor now also clamps max-per-week down to the selected allowed-days count on save (and live-updates a small helper note as allowed days change), so an impossible "6 per week, 3 allowed days" rule can no longer be saved. `PlannerService.generateWithDiagnostics` schedules every enabled must-include activity first: it deterministically (via the existing seeded `Random`) places the activity on all of its allowed weekdays when there are no more of them than `maxPerWeek`, or a shuffled subset of size `maxPerWeek` otherwise, crediting those placements to the same scheduled-count/scheduled-day bookkeeping the flexible picker already uses before any flexible slot is filled - so flexible suggestions only ever fill whatever per-day template capacity is left over, never bump a must-include activity off its guaranteed days, and never double-book it. noConsecutiveDays and difficulty-aware spacing remain soft preferences for flexible fill only; must-include scheduling never consults them, so they can't block a must-include activity from reaching its requested count. `RangePlannerService`'s existing weekly-chunk stitching applies this per chunk for free, so max-per-week (and the must-include guarantee) still resets every generated week across 2-week/month horizons. A new `mustIncludeShortfallCount` diagnostic (surfaced through the existing planner-conflict message) flags the one remaining edge case the editor's clamp doesn't fully prevent: legacy/imported data where `maxPerWeek` exceeds `allowedWeekdays.length`. Manual plan items are untouched by all of this, since they're merged in after generation by the existing `_applyManualItems()` pass. No exact recurring-event scheduling, drag/drop, or date-moving was added.
- [x] Slice 8b — must-include no longer shrinks flexible variety: manual testing of slice 8 found that because must-include placements counted against each day's normal plan-style quota (`targetCount`), a heavily must-include activity (e.g. "Eat Together" 6/week) routinely crowded out flexible suggestions entirely - the day's quota was already "spent" by the must-include item before the flexible picker ever ran, so the plan lost its usual variety. Fixed in `PlannerService.generateWithDiagnostics`: must-include placements are still credited up front and still claim their days first, but the flexible fill loop now always targets the day's full plan-style template count (`targetCount`) regardless of how many must-include items already landed on that day, instead of the previous `max(0, targetCount - mustItemsForDay.length)`. Must-include items are additive baseline content on top of the normal plan, not a deduction from it. The same up-front scheduled-count/scheduled-day bookkeeping that already prevented double-booking continues to do so unchanged - flexible fill still can't re-add a must-include activity on a day it already claimed, or beyond its `maxPerWeek` cap. `targetActivityCount`/`mustIncludeShortfallCount` diagnostics, allowed-weekdays/`enabled=false` handling, the deterministic allowed-days subset selection, noConsecutiveDays-does-not-block-must-include, and the editor's max-per-week clamp are all unchanged. Updated the "Must include in plans" helper text to "The planner adds this first, then still fills the rest of the plan with other activities." to set the right expectation. Manual Add-to-plan items are unaffected either way, since they were already merged in after generation and were never counted against the per-day plan-style quota in the first place (confirmed unchanged, not newly surprising).
- [x] Slice 8c — must-include baseline variety audit and fix: manual testing of slice 8b still showed Month view dominated by "Eat Together" alone on many days. A focused audit (temporary diagnostic script over week/twoWeek/month and Gentle/Balanced/Push, deleted before commit) found the root cause: the plan-style templates (`gentle`/`balanced`/`push`) intentionally include zero-target "rest" days (4 of 7 for gentle, 2 of 7 for balanced/push), and slice 8b's fix still used that raw per-day `targetCount` - 0 on a rest day - as the flexible loop's iteration bound. A must-include item landing on one of those zero-target days (likely, since a 6-day-per-week must-include activity covers most of the week) left the day with only the must-include item and no chance at flexible variety, even with plenty of flexible activities available. Measured before the fix with a 6-day/6-max "Eat Together" plus 6 flexible activities: 49% of days were must-only under Gentle, ~24-25% under Balanced/Push, 47% over a Gentle month horizon. Fixed with the smallest safe change: in `PlannerService.generateWithDiagnostics`, a day with at least one must-include item and a normal `targetCount` of 0 now gets a flexible-fill attempt bound of 1 instead of 0 (`flexibleAttempts = targetCount == 0 && mustItemsForDay.isNotEmpty ? 1 : targetCount`); days without any must-include item keep their normal zero-target rest days untouched, and the bump is capped at exactly 1 extra attempt (never unlimited) and still subject to every existing flexible-fill rule (own `maxPerWeek`, allowed weekdays, no-consecutive-days, difficulty-aware spacing, same-day dedup). Re-running the audit after the fix showed 0% must-only days across all three styles and the month horizon, with `mustIncludeShortfallCount`/must-include `maxPerWeek` guarantees unaffected. `targetActivityCount` diagnostics deliberately stay based on the plan style's raw template sum (unaffected by the bump) since the minimum-flexible-target-of-1 top-up is an opportunistic enrichment, not a new formal target diagnostics should flag as blocked if unfilled; `scheduledActivityCount` naturally reflects the extra flexible items when they're successfully added. Generation remains fully deterministic for the same seed/date/rules. Manual Add-to-plan items are unaffected, since they're merged in after generation and were never part of this per-day target-count calculation.
- [x] Slice 9 — historical calendar archive foundation: the current generated plan is mostly re-derived deterministically from a seed plus the live activity pool/rules, so a source-`Activity` rename, a rule edit, or just reloading after the pool changed could let the next build silently produce different content for a date that already happened, with `_applyOverlays` quietly failing to match stale check-in/lock keys - real history lost, not just stale display. Added `PlanHistoryEntry` (`lib/models/plan_history_entry.dart`): an occurrence-keyed (`yyyy-MM-dd:activityId`, shared with the existing check-in/lock occurrence-key scheme) snapshot of title/timeSlot/durationMinutes/category/difficulty/energy/social/source (`generated`/`manual`/`outsideEvent`)/check-in status/locked/removed/createdAtMillis/updatedAtMillis, independent of the live `Activity`/`ManualPlanItem` it came from. A new `AppState._planHistory` map is upserted by `_archiveCurrentRange()`/`_archiveOccurrence()`/`_upsertArchiveEntry()` after every build (`regenerate`, `generateRange`, `setPlanStyle`, `_applySavedState`, `addManualPlanItem`, `removeManualPlanItem`, and the no-saved-state constructor path) and on every check-in (`notifyCheckIn`), lock toggle (`toggleLock`), removal (`removeFromPlan`), and per-occurrence edit (`editPlannedOccurrence`). Once an entry's date is today or earlier, its snapshot fields (everything except status/locked/removed) freeze on passive rebuilds/check-ins/locks/removals and only refresh on a deliberate `editPlannedOccurrence` call (`forceSnapshotUpdate: true`) - future-dated entries keep refreshing freely since nothing has happened there yet. Entries are never deleted, so regeneration/range/plan-style changes can only add to the archive, never erase a past occurrence already captured. `SavedState.planHistory` and `PersistenceService.savePlanHistoryMap`/the load path round-trip the map through the existing flat local-storage and Firestore calendar-document path (no new Firestore collection, no rules changes) - old saved data with no `planHistory` key defaults to an empty map and loads normally. `AppState.planHistory`/`planHistoryEntryFor(date, activityId)` expose it for tests and future history UI; no browsing UI, year/N-day view, trend charts, or `ProgressSummaryCalculator`/Progress-screen change were added this slice (Progress still reads the live generated/visible window) - see `docs/PARKING_LOT.md`'s "Historical calendar archive and trends" entry for what remains parked on top of this foundation.
