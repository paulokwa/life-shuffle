...
Still to build for MVP 1: calendar create/leave/delete lifecycle, planning-dimensions onboarding, shared-edit/sync conflict messages, one-by-one/week-review check-in views, printable/PDF export, output detail toggles, a deployed rules smoke test for Laura's account, and a real calendar-app subscription test for the public feed.
