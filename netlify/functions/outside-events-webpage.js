...
async function fetchNsccEventApi(eventId, fetchImpl) {
  ...
  if (!response.ok) throw new Error(`NSCC API returned ${response.status}`);
...
function extractEventsFromJson(text, { sourceId, sourceName, sourceUrl, city, rangeStart, rangeEnd }) {
  let data;
  try {
  ...
  if (data && typeof data === "object" && data.Name && data.DateStart) {
    const start = new Date(data.DateStart);
    if (isNaN(start.getTime())) return null;

    const title = data.Name.trim();
    const summary = data.Summary || data.Details || "";
    const venue = data.OffsiteLocation || (data.Locations && data.Locations[0] && data.Locations[0].LongName) || undefined;

    return [{
      id: `${sourceId}-${stableId(`${title}|${start.toISOString()}|${sourceUrl}`)}`,
      title,
      cleanedTitle: title,
      summary: summary.replace(/<[^>]+>/g, " ").slice(0, 280).trim(),
      startDateTimeMillis: start.getTime(),
      endDateTimeMillis: data.DateEnd ? new Date(data.DateEnd).getTime() : undefined,
      venueName: venue,
      sourceName,
      sourceType: "webPage",
      sourceUrl,
      city: city || undefined,
      priceLabel: data.Fee || undefined,
      isFree: data.Fee ? /free/i.test(data.Fee) : undefined,
      tags: inferTags(`${title} ${summary}`),
      confidence: 0.95,
      missingFields: [],
      raw: { userSourceId: sourceId, extractionMode: "deterministic-json" },
...
function extractDeterministicEvents({
  ...
  const jsonEvents = extractEventsFromJson(text, { sourceId, sourceName, sourceUrl, city, rangeStart, rangeEnd });
...
async function handler(event, context) {
  ...
  try {
    ...
    let text;
    const nsccMatch = parsedUrl.href.match(
      /nscc\.ca\/alumni\/get-involved\/events\/eventdetails\.aspx\?eventid=(\d+)/i,
    );
    if (nsccMatch) {
      text = await fetchNsccEventApi(nsccMatch[1], fetchImpl);
    } else {
